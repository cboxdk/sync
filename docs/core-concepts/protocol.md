...
Processing checks global mutation identity first. An exact retry under the same trusted actor/integration context returns the stored immutable result. Changing that trusted identity context also fails as a protocol error. Reusing that ID with changed content, including replica, sequence, operations, `from`, atomic mode, expected revision or resolution, throws `ProtocolException`. An acknowledged sequence arriving under a different ID means the writer is **behind** - its state restored from an older backup - and is answered `mutation_gap` with reason `sequence_behind` and where the stream is, so it renumbers upward. The exception is a position whose receipt was pruned: that could be a replay of a write that was applied, so it is answered `receipt_pruned` instead - final for that write, never renumbered. Fingerprints use internal PHP serialization of normalized request content (independent of PHP object sharing), not a public cross-language wire format. Operation ordering is part of the payload: retries must keep it.
