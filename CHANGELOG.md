...
- `Ledger` gains `amendReceipt()` and `receiptAt()`, the latter reading a stream position's answer as the latest committed state by `(space, replica, sequence)` - a new index - so a locking read inside a host transaction never touches another tenant's range.
...
- **The outbox knows which writes may already be on the server**: one refused as `receipt_pruned` or `protocol_violation`, and any write sent more than once (`OutboxStore::countSend()`/`sends()`), since an earlier attempt got no answer. `requeue()` refuses those without `evenIfItMayHaveLanded`, and `dismiss()` does not abandon the writes that need such a create. `Outbox::relatedBy()` holds the application's references for calls not given them.
