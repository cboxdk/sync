...
- `migrate()` gives receipts from earlier releases their stream position, and a position that already has an answer - a stream and its receipts that disagree after a partial restore - is refused as that, not as a retryable duplicate.
